...
# ---------------------------------------------------------------------------
BOOTSTRAP_SERVERS = os.getenv("KAFKA_BOOTSTRAP_SERVERS", "10.0.2.119:9092,10.0.2.135:9092").split(",")
...
SEND_INTERVAL_SEC = float(os.getenv("SEND_INTERVAL_SEC", "1.0"))   # seconds between sweeps
...
def build_device_registry(n: int) -> list[dict]:
    ...
    for i in range(1, n + 1):
        ...
        devices.append({
            "id":          f"device_{i:04d}",
            "region":      region_name,
            "model":       random.choice(["TempSensor-Pro", "HumidBot-v2", "FleetTracker-X"]),
