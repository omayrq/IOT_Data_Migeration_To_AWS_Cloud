...
TIMESTREAM_TELEMETRY_TABLE = os.environ.get("TIMESTREAM_TELEMETRY_TABLE", "device_telemetry")
...
# ── Clients ───────────────────────────────────────────────────────────────────
timestream = boto3.client("timestream-write", region_name=os.environ.get("AWS_REGION", "us-east-1"))
...
def write_to_timestream(table_name: str, records: list[dict]) -> int:
    ...
    for i in range(0, len(records), batch_size):
        ...
        try:
            ...
            logger.warning(f"Rejected {len(e.response['RejectedRecords'])} records in {table_name}")
...
def ensure_timestream_resources():
    ...
    table_configs = [
        (TIMESTREAM_TELEMETRY_TABLE, 24 * 3600,    365 * 24 * 3600),   # 1 day memory, 1 year magnetic
...
def lambda_handler(event, context):
    ...
    try:
        ...
        telemetry_rows = query_snowflake(conn, """
        ...
        total_written += write_to_timestream(TIMESTREAM_TELEMETRY_TABLE, telemetry_records)
        ...
        anomaly_rows = query_snowflake(conn, """
